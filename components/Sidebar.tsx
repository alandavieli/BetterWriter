...
import { suggestFolderStructure } from '../services/geminiService';
...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  const handleAiSort = async () => {
    setIsSorting(true);
    try {
      const rootChildren = fileMap[rootFolderId]?.children || [];
      ...
      if (topFiles.length < 2) {
        alert("Add more files to the root folder to use AI Sort.");
        setIsSorting(false);
        return;
      }

      const { folders } = await suggestFolderStructure(topFiles);
      onReorganize(folders);
              ...
              >
                ...
                <div className="flex-1 min-w-0 flex items-center gap-2">
                   {node.type === NodeType.FOLDER ? (
                    ...
                    <Icons.FileText size={14} className={isActive ? 'text-gold-500' : 'text-gray-400'} />
                  )}
      ...
      <div className="px-4 mb-2">
        ...
        <div className="grid grid-cols-4 gap-1 p-1 bg-white/50 dark:bg-white/5 rounded-lg border border-gold-100 dark:border-white/5">
          ...
          <button 
            ...
            className={`p-1.5 flex justify-center hover:bg-gold-200 dark:hover:bg-white/10 rounded text-gold-600 ${isSorting ? 'animate-pulse' : ''}`}
            title="AI Sort & Group"
            disabled={isSorting}
          >
             {isSorting ? <span className="animate-spin text-xs">⟳</span> : <Icons.Sparkles size={16} />}
          </button>
        </div>
        <input 
          type="file" 
